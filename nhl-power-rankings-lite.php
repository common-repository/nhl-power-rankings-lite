<?php
/*
Plugin Name: NHL Hockey Power Rankings Lite
...
function NHLSPRLPageURL() {
 $NHLSPRLpageURL = 'http';
 $NHLSPRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $NHLSPRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $NHLSPRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $NHLSPRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_nhlsprlstats() 
{
?>
<h2>NHL Power Rankings Lite</h2>
<?php nhlsprl_stats(); ?>
<?php
}

function nhlsprlstats_install()
{
register_sidebar_widget(__('NHL Power Rankings Lite'), 'widget_nhlsprlstats'); 
}
add_action("plugins_loaded", "nhlsprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'nhlsprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'nhlsprl_stats_remove' );

function nhlsprl_stats_install() 
{
...
add_option("nhlsprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("nhlsprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("nhlsprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NHL Power Rankings Installation", "Hi\n\nLITE NHL Power Rankings Activated at \n\n".NHLSPRLPageURL()."\n\nNHL Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function nhlsprl_stats_remove() 
{
/* Deletes the database field */
delete_option('nhlsprl_scroll_text_color');
delete_option('nhlsprl_scroll_text_color1');
delete_option('nhlsprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'nhlsprl_stats_admin_menu');

function nhlsprl_stats_admin_menu() {

add_options_page('NHL Power Rankings Lite', 'NHL Power Rankings Lite Settings', 'administrator', 'nhl-power-rankings-lite.php', 'nhlsprl_stats_plugin_page');
}
}
function nhlsprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('nhlsprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('nhlsprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('nhlsprl_scroll_text_color2'));

$myads = "http://www.ibet.ws/nhl_power_rankings_magpie_lite/nhl_power_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $myads;
$rss = nhlsprl_fetch_rss( $url );
...
function nhlsprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>NHL Power Rankings Options Page</h2>
...
<p>
<?php echo nhlsprl_stats(); ?>
</p>
<?php echo $myads; ?>
<!-- Start Advanced Plugins List -->
  <h2>If You Want MORE Stats and Information:</h2>
  <p>A93D Offers FREE upgrades for this stats package, that allow you to display advanced and more complete NHL Power Rankings.
  ...
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/nhl-power-rankings.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/nhl-power-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
